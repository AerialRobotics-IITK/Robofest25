...
ardugzswarm: arduimg
	$(RUNTIME) run --rm --net host -it -v "$(PWD)/ardupilot:/ardupilot" ardupilot:latest $(SWARM_ARDU_GZ_CMD)

mavswarm:
	$(RUNTIME) run -it --rm --net host -e MAV_ID -e NUM=$(NUM) -e FCU_URL=$(FCU_URL) -v "$(PWD)/workspace/src:/workspace/src"  $(IMAGE) $(MAV_SWARM)
