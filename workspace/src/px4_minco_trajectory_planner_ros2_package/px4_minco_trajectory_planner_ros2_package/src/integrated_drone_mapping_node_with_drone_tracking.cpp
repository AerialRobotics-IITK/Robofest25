...
private:
  ...
  bool planning_use_odom_start_{true};
  double planning_start_x_{0.0};
  double planning_start_y_{0.0};
  double planning_start_z_{1.0};
  double planning_goal_x_{GOAL_X};
  double planning_goal_y_{GOAL_Y};
  double planning_goal_z_{GOAL_Z};
  ...
  double swarm_peer_timeout_sec_{1.0};
  ...
  /* ================= INIT ================================ */
  /* ========================================================= */
  void init_planning_params() {
    planning_use_odom_start_ =
        declare_parameter<bool>("planning.start.use_odom", true);
    planning_start_x_ = declare_parameter<double>("planning.start.x", 0.0);
    planning_start_y_ = declare_parameter<double>("planning.start.y", 0.0);
    planning_start_z_ = declare_parameter<double>("planning.start.z", 1.0);
    planning_goal_x_ = declare_parameter<double>("planning.goal.x", GOAL_X);
    planning_goal_y_ = declare_parameter<double>("planning.goal.y", GOAL_Y);
    planning_goal_z_ = declare_parameter<double>("planning.goal.z", GOAL_Z);
  }
  ...
  void init_swarm_params() {
    ...
    swarm_peer_timeout_sec_ =
        declare_parameter<double>("swarm.peer_timeout_sec", 1.0);
    ...
    swarm_default_segment_dt_ =
        declare_parameter<double>("swarm.default_segment_dt", 0.1);

    if (swarm_threshold_ <= 0.0) {
      RCLCPP_WARN(get_logger(),
                  "swarm.threshold must be > 0. Falling back to 1.0.");
      swarm_threshold_ = 1.0;
    }
    ...
    for (double &axis : swarm_ellipsoid_diag_) {
      if (axis <= 0.0) {
        RCLCPP_WARN(get_logger(),
                    "swarm.ellipsoid_diag entries must be > 0. "
                    "Falling back to [1, 1, 1].");
        swarm_ellipsoid_diag_ = {1.0, 1.0, 1.0};
        break;
      }
    ...
    if (swarm_peer_timeout_sec_ <= 0.0) {
      RCLCPP_WARN(get_logger(),
                  "swarm.peer_timeout_sec must be > 0. Falling back to 1.0s.");
      swarm_peer_timeout_sec_ = 1.0;
    }

    if (swarm_default_segment_dt_ <= 0.0) {
      RCLCPP_WARN(
          get_logger(),
          "swarm.default_segment_dt must be > 0. Falling back to 0.1s.");
      swarm_default_segment_dt_ = 0.1;
    }
  }
  ...
  void init_peer_traj_subscribers() {
    ...
    for (const auto &topic : swarm_peer_topics_) {
      if (topic == swarm_self_trajectory_topic_) {
        RCLCPP_WARN(get_logger(),
                    "Ignoring peer topic '%s' because it matches "
                    "swarm.self_trajectory_topic.",
                    topic.c_str());
        continue;
      }
    ...
    RCLCPP_INFO(get_logger(),
                "Swarm avoidance enabled. Subscribed to %zu peer trajectory "
                "topics (self topic filtered).",
  ...
  void generate_head_tail(Matrix3d &head, Matrix3d &tail) {
    ...
    if (planning_use_odom_start_) {
      ...
      // Start from configured waypoint with zero start velocity.
      head.col(0) << planning_start_x_, planning_start_y_, planning_start_z_;
      head.col(1).setZero();
    }
    // Acceleration (col 2) stays 0 unless you have filtered IMU data.

    // Goal State: Desired position, stopping at rest (v=0, a=0)
    tail.col(0) << planning_goal_x_, planning_goal_y_, planning_goal_z_;
  }
