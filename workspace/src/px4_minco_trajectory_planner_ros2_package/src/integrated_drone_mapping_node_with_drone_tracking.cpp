...
public:
    IntegratedMappingSFCGCOPTER()
        : Node("integrated_mapping_sfc_gcopter")
    {

        namespace_ = declare_parameter<std::string>("namespace", "/uav1");

        RCLCPP_INFO(get_logger(),
                    "🚀 Mapping → SFC → GCOPTER (Reactive)");
...
private:
    ...
    static constexpr float GRID_W = 20.0f;
    static constexpr float GRID_H = 20.0f;
    static constexpr float RES = 0.05f;

    /* ================= GLOBAL GOAL ================= */

    static constexpr double GOAL_X = 10.0f;
    static constexpr double GOAL_Y = 10.0f;
    static constexpr double GOAL_Z = 1.0f;

    static constexpr double DRONE_RADIUS_CM = 5.0;
    ...
    std::vector<rclcpp::Subscription<nav_msgs::msg::Path>::SharedPtr> peer_traj_subs_;
    ...
    void init_swarm_params()
    {
        ...
        swarm_ellipsoid_diag_ =
            declare_parameter<std::vector<double>>("swarm.ellipsoid_diag", {1.0, 1.0, 1.0});
        swarm_peer_timeout_sec_ = declare_parameter<double>("swarm.peer_timeout_sec", 0.3);
        swarm_peer_topics_ =
            declare_parameter<std::vector<std::string>>("swarm.peer_topics", std::vector<std::string>{});
        swarm_self_trajectory_topic_ =
            declare_parameter<std::string>("swarm.self_trajectory_topic", "/swarm/self_trajectory");

        if (swarm_ellipsoid_diag_.size() != 3)
        {
            RCLCPP_WARN(
                get_logger(),
                "swarm.ellipsoid_diag must have 3 values. Falling back to [1, 1, 1].");
    ...
    Trajectory<3> path_to_trajectory(const nav_msgs::msg::Path &path) const
    {
        ...
        for (size_t i = 0; i + 1 < n; ++i)
        {
            ...
            const Eigen::Vector3d p0(
                p0_msg.pose.position.x,
                p0_msg.pose.position.y,
                p0_msg.pose.position.z);
            const Eigen::Vector3d p1(
                p1_msg.pose.position.x,
    ...
    void peer_path_cb(const nav_msgs::msg::Path::SharedPtr msg, const std::string &topic)
    {
    ...
    void init_peer_traj_subscribers()
    {
        ...
        if (swarm_peer_topics_.empty())
        {
            RCLCPP_WARN(
                get_logger(),
                "swarm.enabled=true but swarm.peer_topics is empty. Swarm penalty will be inactive.");
            return;
        }
    ...
    nav_msgs::msg::Path trajectory_to_path_msg(const Trajectory<3> &traj) const
    {
        ...
        for (int i = 0; i < traj.getPieceNum(); ++i)
        {
            ...
            for (double t = 0.0; t < dur; t += sample_dt)
            {
                ...
                const rclcpp::Time stamp_time = t0 + rclcpp::Duration::from_seconds(t_global);
                const int64_t stamp_ns = stamp_time.nanoseconds();
                ps.header.stamp.sec = static_cast<int32_t>(stamp_ns / 1000000000LL);
                ps.header.stamp.nanosec = static_cast<uint32_t>(stamp_ns % 1000000000LL);
    ...
    void init_mapper()
    {
        ...
        mapper_ =
            std::make_shared<OccupancyMapper>(
                GRID_W, GRID_H, RES, 20.0);
    }
    ...
    void init_sfc()
    {
        ...
        corridor_ =
            std::make_shared<CorridorBuilder>(
                grid_w_, grid_h_, RES,
                RES * 100.0,
                DRONE_RADIUS_CM);
    }
    ...
    void init_ros()
    {
        ...
        state_sub_ = create_subscription<mavros_msgs::msg::State>(
            ...
            std::bind(&IntegratedMappingSFCGCOPTER::mavros_state_cb, this, std::placeholders::_1));

        pos_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
            namespace_ + "/local_position/pose", mavros_qos_,
            std::bind(&IntegratedMappingSFCGCOPTER::pose_cb, this, std::placeholders::_1));

        vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>(
            namespace_ + "/local_position/velocity_local", mavros_qos_,
            std::bind(&IntegratedMappingSFCGCOPTER::vel_cb, this, std::placeholders::_1));

        // Common scan subscription
        scan_sub_ = create_subscription<sensor_msgs::msg::LaserScan>(
            "/uav1/scan",
            rclcpp::QoS(10).best_effort(),
            std::bind(&IntegratedMappingSFCGCOPTER::scan_cb, this, std::placeholders::_1));
    ...
    void init_timer()
    {
        ...
        pipeline_step_timer_ =
            create_wall_timer(
                1s,
                std::bind(
                    &IntegratedMappingSFCGCOPTER::pipeline_step,
                    this));
        ...
        command_loop_timer_ =
            create_wall_timer(
                0.01s,
                std::bind(
                    &IntegratedMappingSFCGCOPTER::command_loop,
                    this));
    }
    ...
    void update_map_step()
    {
        ...
        occ_msg_ =
            mapper_->get_grid_message("map", now());
        ...
        bin_map_ =
            ObstacleInflator::inflateObstacles(
                raw_bin,
                grid_w_, grid_h_,
                RES,
                DRONE_RADIUS_CM);
        ...
        std::ofstream file("binary_map_log.txt"); // overwrite mode
            ...
            if (!file.is_open()) {
                ...
                RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 5000, "bin_map_ logged to binary_map_log.txt in map_data format.");
            }
    ...
    void pipeline_step()
    {
        ...
        global_traj_index_=0;                       // again come back to starting index

        /* ===== BUILD CORRIDOR ===== */

        Vector2i start =
            world_to_grid(head.x(), head.y());

        Vector2i goal =
            world_to_grid(tail.x(), tail.y());
        ...
        if (!file.is_open()) {
            ...
            RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 5000, "SFC successfully logged.");
        }
        ...
        const std::vector<uint8_t> &map)
    {
        ...
        while (index < (lookahead + global_traj_index_) &&
               index < global_traj_.size())
        {

            Vector3d p = global_traj_[index].p;
    ...
    void run_gcopter(
        const std::vector<MatrixX4d> &corridor,
        const Vector3d &head_pos,
        const Vector3d &tail_pos)
    {
        ...
        if (!sfc.setup(
                0.7, head, tail, corridor,
                0.25, 0.08, 4,
                bounds, weights, params))
        {

            RCLCPP_ERROR(
                get_logger(),
                "GCOPTER setup failed");
        ...
        if (swarm_enabled_)
        {
            std::vector<Trajectory<3>> other_agents = collect_fresh_peer_trajectories();
            ...
            if (other_agents.empty())
            {
                RCLCPP_WARN(
                    get_logger(),
                    "Swarm penalty enabled but no fresh peer trajectories. Planning without swarm repulsion.");
            }
        }
        ...
        if (self_traj_path_pub_)
        {
            ...
            RCLCPP_INFO(
                ...
                swarm_self_trajectory_topic_.c_str(),
                my_path.poses.size());
        }
        ...
        double zmax)
    {
        ...
        for (int i = 0; i < A.rows(); i++)
        {
            ...
            double d =
                -b(i) + A(i, 0) * ox * scale + A(i, 1) * oy * scale;
    ...
    {
        // 1. Safety Check: If we don't know where the drone is, we can't compute error.
        if (!has_odom_) {
        ...
        if (global_traj_index_ >= 0 && static_cast<size_t>(global_traj_index_) < global_traj_.size()) 
        {
        ...
        else 
        {
            ...
            error_vx = 0; error_vy = 0; error_vz = 0;
            return;
        }
    ...
    void tracking_pp_controller(double &ax, double &ay, double &az)
    {
        ...
        if (global_traj_index_ < 0 || static_cast<size_t>(global_traj_index_) >= global_traj_.size())
        {
            ...
            RCLCPP_INFO_THROTTLE(
                this->get_logger(), *this->get_clock(), 2000,
                "tracking_pp_controller(): trajectory finished, dist->goal=%.3f", dist);

            // We want to hover at the last point. If already very close -> GOAL_REACHED
            const double GOAL_THRESH = 0.15; // meters
            if (dist < GOAL_THRESH)
            {
            ...
            {
                // Trajectory finished but not yet at goal -> hover at last trajectory point
                RCLCPP_INFO_THROTTLE(
                    this->get_logger(), *this->get_clock(), 2000,
                    "tracking_pp_controller(): holding last trajectory point -> switching to HOVER");
        ...
        const double KV_VEL = 0.2;

        double ref_vx = 0.0, ref_vy = 0.0, ref_vz = 0.0;
        if (global_traj_index_ >= 0 && static_cast<size_t>(global_traj_index_) < global_traj_.size())
        {
            ref_vx = global_traj_[global_traj_index_].v.x();
            ref_vy = global_traj_[global_traj_index_].v.y();
            ref_vz = global_traj_[global_traj_index_].v.z();
        }

        double cmd_vx = ref_vx + KP_POS * ex + KV_VEL * evx;
        double cmd_vy = ref_vy + KP_POS * ey + KV_VEL * evy;
        double cmd_vz = ref_vz + KP_POS * ez + KV_VEL * evz;

        const double MAX_VEL = 2.0;
        cmd_vx = std::clamp(cmd_vx, -MAX_VEL, MAX_VEL);
        cmd_vy = std::clamp(cmd_vy, -MAX_VEL, MAX_VEL);
        cmd_vz = std::clamp(cmd_vz, -MAX_VEL, MAX_VEL);
        ...
        RCLCPP_DEBUG_THROTTLE(
            ...
            "Errors: Pos(%.3f %.3f %.3f) Vel(%.3f %.3f %.3f) | VelCmd(%.3f %.3f %.3f)",
            ex, ey, ez, evx, evy, evz, ax, ay, az);
    }
    ...
    void command_loop()
    {
        ...
        for (size_t idx = 0; idx < bin_map_.size(); ++idx) {
            ...
            if (bin_map_[idx] == 1) {
                ...
                // We add (resolution / 2.0) to get the center of the cell, rather than the bottom-left corner.
                // Vector2d world = grid_to_world(grid_x, grid_y);
                ...
                //std::cout << "   2. Grid Index : [X: " << grid_x << ", Y: " << grid_y << "]" << std::endl;
                //std::cout << "   3. World Pos  : (X: " << world_x << ", Y: " << world_y << ")" << std::endl;
                //std::cout << "-----------------------------" << std::endl;
            }
        }
    ...
    {
        // Wait until MAVROS state + odom are valid, then mirror yaw_body_tracking logic:
        // SET_MODE -> ARM -> TAKEOFF (GUIDED takeoff to TAKEOFF_ALTITUDE), then switch to TRACKING.
    ...
    void handle_takeoff()
    {
        ...
        if (z >= TAKEOFF_ALTITUDE - 0.1)
        {
            ...
            RCLCPP_INFO(
                get_logger(),
                "TAKEOFF: reached altitude -> TRACKING (z=%.2f)",
                z);
    ...
    void handle_hover()
    {
        ...
        RCLCPP_INFO_THROTTLE(
            ...
            *this->get_clock(),
            2000,
            "Entered HOVER mode at (%.2f, %.2f, %.2f)",
            hover_x_, hover_y_, hover_z_);

        // In hover we simply command zero velocity; position is held by the autopilot
        publish_stop_velocity();
    }
