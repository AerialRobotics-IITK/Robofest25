...
private:
    ...
    rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr lateral_sub_;
    ...
    // Lateral command factor from tracker (-1.0 .. 1.0)
    float lateral_cmd_ = 0.0f;
    ...
    void init_subscribers()
    {
        ...
        lateral_sub_ = create_subscription<std_msgs::msg::Float32>(
            "/lateral_command", 10,
    ...
    void lateral_callback(const std_msgs::msg::Float32::SharedPtr msg)
    {
    ...
    void handle_hover()
    {
        ...
        if ((vision_alive() && std::abs(waist_error_) > WAIST_THRESHOLD_ENTER) || lateral_cmd_ != 0)
        {
            ...
            RCLCPP_INFO(
                get_logger(),
                "HOVER: |waist_error|=%.1f > %.1f or lateral_cmd=%.1f -> YAW_CONTROL",
    ...
    void publish_yaw_velocity(double yaw_cmd)
    {
        ...
        // 2. LATERAL MOVEMENT (POSITION CONTROL)
        double Kp_xy = 1.5; // Tuning: Higher means it snaps to the position faster
        ...
        // (This ensures "Left" is always relative to where the camera is pointing)
        double yaw = vehicle_odom_.orientation.yaw;
        double offset_x_global = x_offset_body * std::cos(yaw) - y_offset_body * std::sin(yaw);
        double offset_y_global = x_offset_body * std::sin(yaw) + y_offset_body * std::cos(yaw);

        // Calculate the exact global coordinate the drone needs to fly to
        double target_x = hover_reference_.position.x + offset_x_global;
        double target_y = hover_reference_.position.y + offset_y_global;

        // Calculate distance to the target
        double x_error = target_x - vehicle_odom_.position.x;
        double y_error = target_y - vehicle_odom_.position.y;

        // Apply Proportional Control with a speed limit (so it doesn't jerk violently)
        double max_speed = 1.0; // Maximum speed in m/s while translating
        twist.twist.linear.x = std::clamp(Kp_xy * x_error, -max_speed, max_speed);
        twist.twist.linear.y = std::clamp(Kp_xy * y_error, -max_speed, max_speed);

        RCLCPP_INFO(get_logger(), "Yaw command: %f rad/s X_velocity: %f Y_velocity: %f Z_velocity: %f", yaw_cmd, twist.twist.linear.x, twist.twist.linear.y, twist.twist.linear.z);
