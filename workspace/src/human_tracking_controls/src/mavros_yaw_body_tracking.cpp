...
private:
    ...
    void handle_hover()
    {
        ...
        if (vision_alive() && std::abs(waist_error_) > WAIST_THRESHOLD_ENTER)
        {
            pid_yaw_.reset();
            RCLCPP_INFO(
                get_logger(),
                "HOVER: |waist_error|=%.1f > %.1f -> YAW_CONTROL",
                std::abs(waist_error_), WAIST_THRESHOLD_ENTER);
    ...
    void handle_yaw_control()
    {
        ...
        if (std::abs(waist_error_) < WAIST_THRESHOLD_EXIT)
        {
            RCLCPP_INFO(
                get_logger(),
